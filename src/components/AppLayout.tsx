...
const NAV_ICON_MAP: { id: AppTab; key: string; icon: string }[] = [
  { id: 'posts', key: 'posts', icon: '◈' },
  { id: 'schedule', key: 'schedule', icon: '◉' },
  { id: 'planner', key: 'planner', icon: '⊟' },
  { id: 'calendar', key: 'calendar', icon: '▦' },
  { id: 'news', key: 'news', icon: '◎' },
  { id: 'ideas', key: 'ideas', icon: '✦' },
...
export default function AppLayout({ user, currentTab, onTabChange, onLogout, demoMode, autoGenerate, onAutoGenerateDone }: Props) {
  ...
  const langToggleStyle: CSSProperties = {
    display: 'inline-flex',
    alignItems: 'center',
    background: 'var(--s2)',
    border: '1px solid var(--bd)',
  ...
  const langBtnStyle = (active: boolean): CSSProperties => ({
    padding: '4px 10px',
    border: 'none',
    background: active ? 'rgba(107,79,255,.2)' : 'none',
    ...
    <div style={rootStyle}>
      ...
      <div style={mainStyle}>
        ...
        <header style={topbarStyle}>
          <span style={topbarTitleStyle}>{t(`tab_${currentTab}`)}</span>
          <div style={searchStyle}>
            ...
            <div style={pillStyle(!!hasLinkedIn)}>
              ...
              {hasLinkedIn ? t('li_linked') : t('li_unlinked')}
            </div>
